      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur border-b border-slate-200">
          <div className="px-6 lg:px-8 py-4">
            ...
            <p className="text-xs text-slate-500">Matchmaker • Integrity detector • Formatting agent • Submission auditor</p>
          </div>
        </header>
        <main className="px-6 lg:px-8 py-6 max-w-[1400px] w-full mx-auto space-y-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-auto text-xs font-bold px-2 py-1 rounded-full bg-slate-900 text-white">4 matches</span>
            </div>
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
              {journals.map(j=>(
                <div key={j.name} className="rounded-2xl border border-slate-200 bg-white p-4 hover:shadow-card-lg transition">
                  <div className="text-sm font-bold leading-tight">{j.name}</div>
                  <div className="text-xs text-slate-500 mt-1">{j.scope}</div>
                  <div className="mt-3 flex gap-1.5 flex-wrap">
                    ...
                    <span className="rounded-full bg-slate-50 border border-slate-200 px-2 py-1 text-xs font-bold">{j.quartile}</span>
                    <span className="rounded-full bg-amber-50 border border-amber-200 px-2 py-1 text-xs font-bold">{j.apc}</span>
                  </div>
                  ...
                  <button className="mt-3 w-full rounded-full bg-slate-900 py-2 text-xs font-bold text-white">Prepare for this journal</button>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
              ...
              <div className="mt-3 space-y-2 text-xs">
                {[
                  ...
                  <div key={k} className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
                    <div><div className="font-bold">{k}</div><div className="text-slate-600">{v}</div></div><div className={`h-2 w-2 rounded-full ${s==='ok' ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                  </div>
                ))}
              </div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                {[
                  ...
                  <div key={k} className={`rounded-xl border p-3 ${s==='ok' ? 'bg-emerald-50 border-emerald-200' : 'bg-amber-50 border-amber-200'}`}>
                    <div className="font-bold">{k}</div><div>{v}</div>
                  </div>
                ))}
              </div>
              <button className="mt-4 w-full rounded-full bg-slate-900 py-2.5 text-xs font-bold text-white">Format for Computers & Education</button>
              <div className="mt-2 text-xs text-slate-500">Auto: structure, headings, ref style, title page, declarations, funding, COI, ethics, data availability.</div>
            </div>
          </div>
        </main>
      </div>
