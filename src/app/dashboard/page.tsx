...
export default function DashboardPage() {
    ...
    const unsub = onAuthStateChanged(firebaseAuth as any, async (u) => {
      ...
      try {
        ...
        try {
          ...
          setUploadedLibrary(libSnap.docs.map(d=>({id:d.id, ...d.data()} as any)));
        } catch {}
        const q = query(
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadStatus(`Uploading to Firebase Storage and extracting...`);
    try {
      ...
      for (const file of Array.from(files)) {
        const storageRef = ref(firebaseStorage as any, `users/${user.uid}/library/${Date.now()}-${file.name}`);
        await uploadBytes(storageRef, file);
        const url = await getDownloadURL(storageRef);
        const docRef = await addDoc(collection(firebaseDb as any, `users/${user.uid}/library`), {
        ...
        uploaded.push({ id: docRef.id, title: file.name, fileUrl: url, authors: user.displayName || user.email, year: new Date().getFullYear().toString() });
      }
      setUploadedLibrary(prev => [...uploaded, ...prev]);
      setUploadStatus(`Successfully imported ${uploaded.length} file(s): ${names.join(", ")} to Firebase Storage. View in Library.`);
    } catch (err: any) {
      setUploadStatus(`Upload failed: ${err.message}. Check Firestore Rules allow read, write: if request.auth != null; and Storage Rules.`);
    } finally {
        ...
        <PageTransition>
          <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-xl border-b border-pink-50">
            <div className="px-4 sm:px-6 lg:px-8 py-4 sm:py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h1 className="text-[26px] sm:text-[28px] font-black tracking-tight leading-none flex items-center gap-3">
                  <img
                ...
                <p className="text-[18px] sm:text-[20px] text-black mt-1.5 font-medium">
              ...
              <div className="flex items-center gap-2 self-start sm:self-auto">
                <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 border border-emerald-200 px-3.5 py-1.5 text-[16px] font-bold text-emerald-700">
                  <ShieldCheck className="h-4 w-4" /> Verified References (APA 7th)
                </span>
                <Link
                  href="/literature"
                  className="inline-flex items-center gap-2 rounded-full bg-ink-900 px-5 py-2.5 text-white text-[17px] font-bold hover:bg-moni-600 transition"
                >
          ...
          <main className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-[1240px] w-full mx-auto space-y-8">
            ...
            <div className="rounded-3xl border border-pink-100 bg-gradient-to-br from-white via-[#FFFEFE] to-moni-50/40 p-6 sm:p-8 shadow-sm">
              <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                <div>
                  <div className="inline-flex items-center gap-2 rounded-full border border-pink-200 bg-moni-50 px-3.5 py-1 text-[15px] font-extrabold text-moni-700 uppercase tracking-wider">
                    <Sparkles className="h-3.5 w-3.5 text-moni-600" /> 15-Step Research Guide
                  </div>
                  <h2 className="mt-3 text-[26px] sm:text-[30px] font-black tracking-tight leading-tight">
                    From your first idea to a finished, well-supported paper
                  </h2>
                  <p className="mt-2 text-[18px] sm:text-[20px] text-black font-medium max-w-[680px] leading-relaxed">
                ...
                <div className="flex flex-wrap gap-2.5">
                  <Link
                    href="/agent-mode"
                    className="inline-flex items-center gap-2 rounded-full bg-ink-900 px-6 py-3.5 text-[18px] font-bold text-white hover:bg-moni-600 transition shadow-lg shadow-ink-900/10"
                  >
                    <Layers className="h-4 w-4" /> Open Agent Mode SOP
                  </Link>
                  <Link
                    href="/literature"
                    className="inline-flex items-center gap-2 rounded-full border border-pink-200 bg-white px-6 py-3.5 text-[18px] font-bold text-black hover:bg-moni-50 transition"
                  >
              ...
              <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 pt-6 border-t border-pink-100">
                {[
                  ...
                  >
                    <div className="text-[14px] font-black uppercase tracking-wider text-moni-600">
                      Phase {idx + 1}
                    </div>
                    <div className="mt-1 text-[17px] font-extrabold text-black group-hover:text-moni-700">
                      {title}
                    </div>
                    <div className="text-[15px] text-black font-medium mt-0.5">{desc}</div>
                  </Link>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-pink-100 bg-white p-6 sm:p-7 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                <div>
                  <div className="text-[21px] font-extrabold text-black">
              ...
              <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto justify-start md:justify-end">
                <label className="cursor-pointer inline-flex items-center gap-2 rounded-full bg-moni-600 px-5 py-2.5 text-[17px] font-bold text-white hover:bg-moni-700 transition shadow-sm">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-pink-200 bg-white px-4 py-2.5 text-[17px] font-bold text-black hover:bg-moni-50 transition"
                >
            ...
            {isProcessing && (
              <div className="rounded-2xl bg-moni-50 border border-moni-200 p-4 flex items-center gap-3 text-[18px] font-bold text-moni-700 animate-pulse">
            ...
            {!isProcessing && uploadStatus && (
              <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-4 flex items-center gap-3 text-[18px] font-bold text-emerald-800">
            ...
            <div className="rounded-3xl border border-pink-100 bg-white p-6 sm:p-7 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-pink-50 pb-4">
                <div>
                  <h2 className="text-[24px] font-black tracking-tight">Your Uploaded Research Library ({uploadedLibrary.length} Documents)</h2>
                ...
                <Link
                  href="/agent-mode"
                  className="inline-flex items-center gap-2 rounded-full bg-ink-900 px-5 py-2.5 text-white text-[17px] font-bold hover:bg-moni-600 transition shrink-0"
                >
                  ...
                  >
                    <div className="space-y-1 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-[15px] font-black uppercase text-moni-600 bg-moni-50 border border-pink-100 px-2.5 py-0.5 rounded-full">
                          {doc.id}
                        </span>
                        <span className="text-[15px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-0.5 rounded-full">
                          APA 7th • DOI Verified
                        </span>
                      </div>
                      <div className="text-[20px] font-extrabold text-black pt-1">{doc.title}</div>
                    ...
                    <div className="flex items-center gap-2.5 shrink-0">
                      {doc.pdfUrl && doc.pdfUrl !== "#" && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1.5 rounded-xl border border-pink-200 bg-white px-4 py-2 text-[16px] font-bold text-black hover:bg-moni-50 transition"
                        >
                          Open PDF
                        </a>
                      )}
                      <button
                        ...
                        className="rounded-xl border border-pink-100 px-4 py-2 text-[16px] font-bold text-moni-600 hover:bg-red-50 hover:text-red-600 transition"
                      >
                        Remove
                      </button>
                    </div>
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between">
                        <span className="inline-flex items-center gap-1.5 rounded-full bg-moni-50 border border-pink-200 px-3 py-1 text-[15px] font-black text-moni-700 uppercase tracking-wider">
                      ...
                      <h3 className="mt-4 text-[24px] font-black tracking-tight text-black group-hover:text-moni-700">
                        {l.title}
                      </h3>
                      <p className="mt-2 text-[18px] text-black font-medium leading-relaxed">{l.desc}</p>
                    </div>
                    <div className="mt-6 pt-4 border-t border-pink-50 flex items-center justify-between text-[16px] font-bold text-moni-600">
            ...
            <div className="rounded-3xl border border-pink-100 bg-white p-6 sm:p-8 shadow-sm">
              <div className="flex items-center justify-between border-b border-pink-50 pb-4">
                <div>
                  <h2 className="text-[24px] font-black tracking-tight text-black">Active Research Projects</h2>
                  <p className="text-[18px] text-black font-medium mt-0.5">
                    Your workspaces are persisted under your Firebase account.
                  </p>
                </div>
                <Link
                  href="/literature"
                  className="text-[18px] font-bold text-moni-600 hover:text-moni-700 inline-flex items-center gap-1.5"
                >
              ...
              {createdProject ? (
                <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50/50 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div>
                    <div className="inline-flex items-center gap-2 text-[17px] font-bold text-emerald-800">
                      <CheckCircle2 className="h-5 w-5 text-emerald-600" /> Initialized 15-Stage Project Workspace
                    </div>
                    <div className="mt-1 text-[22px] font-black text-black">{createdProject}</div>
                    <div className="text-[17px] text-black font-medium">
                  ...
                  <Link
                    href="/agent-mode"
                    className="rounded-full bg-ink-900 px-6 py-3 text-[17px] font-bold text-white hover:bg-moni-600 transition shrink-0"
                  >
                    Open in Agent Mode
                  </Link>
                </div>
                ...
                <div className="mt-6 rounded-2xl border border-pink-100 bg-[#FFFEFE] p-6 sm:p-7">
                  <div className="max-w-[640px]">
                    ...
                    <p className="text-[17px] text-black font-medium mt-1">
                    ...
                    <form onSubmit={handleCreateQuickProject} className="mt-4 flex flex-col sm:flex-row gap-2.5">
                      <input
                        ...
                        className="flex-1 rounded-full border border-pink-200 bg-white px-5 py-3 text-[18px] text-black outline-none focus:border-moni-500 focus:ring-4 focus:ring-moni-100"
                      />
                      <button
                        type="submit"
                        className="rounded-full bg-ink-900 px-6 py-3 text-[18px] font-bold text-white hover:bg-moni-600 transition shrink-0"
                      >
                        Initialize Project
                      </button>
                    </form>
