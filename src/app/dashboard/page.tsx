      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-xl border-b border-pink-50">
          <div className="px-6 lg:px-8 py-5 flex items-start justify-between gap-4">
            <div>
              <h1 className="text-[24px] font-black tracking-tight leading-none">Dashboard</h1>
            ...
            <Link href="/literature" className="hidden md:inline-flex items-center gap-2 rounded-full bg-ink-900 px-5 py-2.5 text-white text-[18px] font-bold hover:bg-moni-600 transition">
        ...
        <main className="px-6 lg:px-8 py-8 max-w-[1080px] w-full mx-auto space-y-8">
          ...
          <div className="rounded-[24px] border border-pink-100 bg-white p-6 lg:p-7 flex flex-col lg:flex-row gap-6 items-start justify-between">
            <div>
              <h2 className="text-[20px] font-black tracking-tight">Continue where you left off</h2>
              <p className="text-[18px] text-black mt-1.5 leading-relaxed max-w-[560px] font-medium">
                Pick up your literature, see what gaps emerged, or open your manuscript. Your progress is saved automatically.
              </p>
              <div className="mt-5 flex flex-wrap gap-2">
                <Link href="/literature" className="inline-flex items-center gap-2 rounded-full bg-ink-900 px-4 py-2 text-[18px] font-bold text-white hover:bg-moni-600"><Search className="h-4 w-4" /> Search literature</Link>
                <Link href="/writing" className="inline-flex items-center gap-2 rounded-full border border-pink-100 bg-white px-4 py-2 text-[18px] font-bold hover:bg-moni-50"><PenLine className="h-4 w-4" /> Open manuscript</Link>
              </div>
            </div>
            <div className="rounded-2xl bg-[#FFF8FB] border border-pink-100 p-4 min-w-[260px]">
              <div className="text-[16px] font-bold tracking-widest uppercase text-moni-500">What’s inside</div>
              <ul className="mt-2 space-y-1.5 text-[18px] font-medium">
          ...
          <section>
            <div className="flex items-center justify-between">
              ...
              <Link href="/literature" className="text-[18px] font-bold text-moni-600 hover:text-moni-700 inline-flex items-center gap-1">Create project <ArrowRight className="h-3.5 w-3.5" /></Link>
            </div>

            {loading ? (
              <div className="mt-4 rounded-2xl border border-pink-50 bg-white p-8 text-[18px] text-black">Loading your workspace…</div>
            ) : !user ? (
              <div className="mt-4 rounded-[20px] border border-pink-100 bg-white p-8 lg:p-10 text-center">
                ...
                <h4 className="mt-4 font-black text-[18px]">Sign in to see your projects</h4>
                <p className="text-[18px] text-black mt-1 max-w-[420px] mx-auto">Your projects live in Firebase under your account. No demo data - just your real work.</p>
                <div className="mt-5 flex justify-center gap-2">
                  <Link href="/login" className="rounded-full bg-ink-900 px-5 py-2.5 text-white text-[18px] font-bold">Log in</Link>
                  <Link href="/signup" className="rounded-full border border-pink-100 px-5 py-2.5 text-[18px] font-bold">Create account</Link>
                </div>
              </div>
              ...
              <div className="mt-4 rounded-[20px] border border-dashed border-pink-200 bg-[#FFFEFE] p-8 lg:p-10 text-center">
                ...
                <h4 className="mt-4 font-black text-[18px]">No projects yet - let’s make your first</h4>
                <p className="text-[18px] text-black mt-1 max-w-[480px] mx-auto font-medium">Start with anything: an idea, a pile of PDFs, or a draft. MONIRESH will keep the structure for you.</p>
                <div className="mt-6 grid sm:grid-cols-3 gap-3 max-w-[720px] mx-auto text-left">
                  {[
                    ...
                    <Link key={t} href="/literature" className="rounded-2xl border border-pink-100 bg-white p-4 hover:border-moni-200 hover:bg-moni-50 transition">
                      ...
                      <div className="text-[16px] text-black">{d}</div>
                    </Link>
                  ))}
                </div>
              ...
              <div className="mt-4 grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {projects!.map((p) => (
                  <Link key={p.id} href="/writing" className="rounded-2xl border border-pink-100 bg-white p-5 hover:border-moni-200 hover:shadow-sm transition">
                    <div className="flex items-center gap-2 text-[16px] text-black"><Clock className="h-3.5 w-3.5" /> Recently updated</div>
                    <h4 className="mt-2 font-bold leading-tight line-clamp-2">{p.title}</h4>
                    <div className="mt-3 inline-flex items-center gap-1 text-[16px] font-bold text-moni-600">Open <ArrowRight className="h-3.5 w-3.5" /></div>
                  </Link>
                ))}
              </div>
            )}
          ...
          <section className="grid lg:grid-cols-3 gap-4">
            <Link href="/literature" className="rounded-2xl border border-pink-100 bg-white p-5 hover:bg-moni-50 transition">
              ...
              <p className="text-[18px] text-black leading-relaxed">Search or upload. We keep your query for the appendix.</p>
            </Link>
            <Link href="/systematic" className="rounded-2xl border border-pink-100 bg-white p-5 hover:bg-moni-50 transition">
              ...
              <p className="text-[18px] text-black leading-relaxed">Include, exclude, maybe. Export PRISMA in one click.</p>
            </Link>
            <Link href="/gaps" className="rounded-2xl border border-pink-100 bg-white p-5 hover:bg-moni-50 transition">
              ...
              <p className="text-[18px] text-black leading-relaxed">Turn patterns into a gap you can defend, then write with sources at hand.</p>
            </Link>
          </section>
