  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <div className="mx-auto w-full max-w-[1080px] px-6 lg:px-8 h-[64px] flex items-center justify-between border-b border-pink-50">
        ...
        <Link href="/signup" className="text-[20px] font-bold hover:text-moni-600">Need an account? Sign up</Link>
      </div>

      <div className="flex-1 flex items-center justify-center p-6">
        <form onSubmit={onSubmit} className="w-full max-w-[380px]">
          <h1 className="text-[30px] font-black tracking-tight leading-none">Welcome back</h1>
          <p className="text-[20px] text-black mt-2">Your research is where you left it.</p>

          {err && <div className="mt-4 rounded-xl bg-red-50 border border-red-100 text-red-700 px-4 py-3 text-[20px]">{err}</div>}

          <label className="block text-[20px] font-bold mt-6">Email</label>
          ...
          <input value={password} onChange={e=>setPassword(e.target.value)} type="password" required placeholder="••••••••" className="mt-1.5 w-full rounded-xl border border-pink-100 bg-white px-4 py-3 text-[20px] outline-none focus:border-moni-300 focus:ring-4 focus:ring-moni-50" />
          ...
          <p className="text-[20px] text-center mt-6 text-black">New here? <Link href="/signup" className="font-bold text-black underline decoration-pink-200 underline-offset-4">Create an account</Link></p>
          <p className="text-[18px] text-center mt-3 text-black"><Link href="/" className="underline">Back to homepage</Link></p>
        </form>
      </div>
    </div>
  );
