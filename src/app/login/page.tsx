    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 grid place-items-center p-6">
        <form onSubmit={onSubmit} className="w-full max-w-[360px] card p-6">
          ...
          <p className="text-[14px] text-[#64748B] mt-1">Sign in to your workspace.</p>
          ...
          <input value={password} onChange={e=>setPassword(e.target.value)} type="password" required placeholder="••••••••" className="mt-1.5 w-full rounded-[8px] border border-[#E2E8F0] px-3 py-2.5 text-[14px] outline-none focus:border-[#4F46E5] focus:ring-1 focus:ring-[#0A0A0A]" />
          <button disabled={loading} className="btn-primary w-full mt-6 !py-2.5">{loading ? "Signing in..." : "Sign in"}</button>
          <p className="text-[13px] text-center mt-4 text-[#64748B]">New here? <Link href="/signup" className="font-medium text-[#1E293B] underline">Create account</Link></p>
        </form>
      </div>
    </div>
  );
