        ...
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur border-b border-slate-200">
          <div className="px-6 lg:px-8 py-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500">Killer feature: structured literature → gap types → verification → novelty score</p>
            </div>
            <div className="hidden md:flex items-center gap-2 text-xs">
              <span className="rounded-full bg-amber-50 border border-amber-200 px-3 py-1 font-bold text-amber-700">3 opportunities • 163 papers analyzed</span>
            </div>
          </div>
        </header>
        <main className="px-6 lg:px-8 py-6 max-w-[1400px] w-full mx-auto space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
            ...
            <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="rounded-xl border border-slate-200 p-4">
                <div className="text-xs font-bold tracking-widest uppercase text-slate-500">Theory distribution</div>
                <div className="mt-3 space-y-2">
                  {[
                    ...
                    <div key={r.t} className="flex items-center gap-2 text-xs">
                      ...
                      <div className="flex-1 h-2 rounded-full bg-slate-100 overflow-hidden"><div className="h-full bg-slate-900" style={{width:r.w}} /></div>
              ...
              <div className="rounded-xl border border-slate-200 p-4">
                <div className="text-xs font-bold tracking-widest uppercase text-slate-500">Geography</div>
                <div className="mt-3 space-y-2">
                  {[
                    { c: "USA", v: "███████████ 312" },
                    { c: "China", v: "█████████ 284" },
                    ...
                    <div key={r.c} className={`flex items-center gap-2 text-xs font-mono ${r.highlight ? 'text-amber-700 font-bold' : 'text-slate-600'}`}><span className="w-16 font-sans font-bold">{r.c}</span> {r.v} {r.highlight && '← GAP'}</div>
                  ))}
                </div>
              </div>
              <div className="rounded-xl border border-slate-200 p-4">
                <div className="text-xs font-bold tracking-widest uppercase text-slate-500">Population</div>
                <div className="mt-3 space-y-2">
                  {[
                    { p: "Students", n: 420 },
                    { p: "Lecturers", n: 132 },
                    ...
                    <div key={r.p} className="flex items-center gap-2 text-xs">
                      ...
                      <div className="flex-1 h-2 rounded-full bg-slate-100"><div className="h-full bg-amber-500" style={{width: `${r.n/4.2}%`}} /></div>
                      <span className="font-mono">{r.n}</span>
                    </div>
                  ))}
                </div>
                <div className="mt-3 rounded-lg bg-amber-50 border border-amber-200 p-2 text-xs"><span className="font-bold">Gap:</span> Lecturers under-studied vs students (3.2×).</div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-1 space-y-3">
              ...
              {mockGaps.map(g=>(
                <button key={g.id} onClick={()=>setSelected(g)} className={`w-full text-left rounded-2xl border p-4 ${selected.id===g.id ? 'bg-slate-900 text-white border-slate-900' : 'bg-white border-slate-200 hover:border-slate-300'}`}>
                  <div className="text-sm font-bold leading-tight">{g.title}</div>
                  <div className={`text-xs mt-1 ${selected.id===g.id ? 'text-white/70' : 'text-slate-500'}`}>{g.evidence}</div>
                  <div className="mt-2 flex gap-1">
                    <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${selected.id===g.id ? 'bg-white text-slate-900' : 'bg-slate-100 border border-slate-200'}`}>{g.overall}/100</span>
                    <span className={`text-[11px] ${selected.id===g.id ? 'text-white/60' : 'text-slate-500'}`}>{g.geography} • {g.method}</span>
                  </div>
                </button>
              ))}
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
                ...
                <div className="mt-4 grid grid-cols-4 gap-2 text-center">
                  {[
                    ...
                    <div key={s.k} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                      <div className="text-lg font-black">{s.v}</div>
                      <div className="text-[10px] font-bold tracking-widest uppercase text-slate-500">{s.k}</div>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
                    ...
                    <div className="mt-1 flex flex-wrap gap-1.5">
                      {["Population","Geographic","Methodological","Variable"].map(t=>(
                        <span key={t} className="rounded-full bg-white border border-slate-200 px-2.5 py-1 text-xs font-bold">{t}</span>
                      ))}
                    </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
                ...
                <div className="mt-3 overflow-auto">
                  <table className="w-full text-xs">
                    <thead><tr className="text-left text-slate-500 border-b"><th className="pb-2 font-bold">Study</th><th className="pb-2 font-bold">Theory</th><th className="pb-2 font-bold">Finding</th><th className="pb-2 font-bold">Gap</th></tr></thead>
                    <tbody>
                      {[
                        ...
                        <tr key={r[0]} className="border-b border-slate-100"><td className="py-2 font-bold">{r[0]}</td><td className="py-2">{r[1]}</td><td className="py-2 font-mono">{r[2]}</td><td className="py-2 text-amber-700">{r[3]}</td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
                <div className="mt-3 flex gap-2">
                  <button className="rounded-full bg-slate-900 px-4 py-2 text-xs font-bold text-white">Export Excel</button>
                  <button className="rounded-full border border-slate-200 px-4 py-2 text-xs font-bold">Export BibTeX</button>
                </div>
              </div>
            </div>
          </div>
