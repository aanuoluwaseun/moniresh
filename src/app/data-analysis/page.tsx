      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur border-b border-slate-200">
          <div className="px-6 lg:px-8 py-4">
            ...
            <p className="text-xs text-slate-500">Upload CSV/Excel/SPSS • Assumption checker • Python/R generation • No invented statistics</p>
          </div>
        </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-white p-8 text-center">
                ...
                <div className="text-xs text-slate-500">CSV, Excel, SPSS (.sav), Stata (.dta), R (.rds) — max 100MB. Data stays in your project, RLS protected.</div>
                <button className="mt-4 rounded-full bg-slate-900 px-6 py-2.5 text-xs font-bold text-white">Browse & Profile Data</button>
                <div className="mt-4 grid grid-cols-3 gap-2 text-xs text-left">
                  {[
                    ...
                    <div key={t} className="rounded-xl border border-slate-100 bg-slate-50 p-3"><div className="font-bold">{t}</div><div className="text-slate-600 leading-tight mt-1">{d}</div></div>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
                ...
                <p className="text-xs text-slate-500 mt-1">Analysis → Code → Output → Interpretation. Reproducible.</p>
                <div className="mt-3 rounded-xl bg-slate-900 text-slate-100 p-3 font-mono text-xs leading-relaxed">
                ...
                <div className="mt-2 rounded-xl bg-slate-50 border p-3 font-mono text-xs">
                  <div className="opacity-60"># R</div>
                  <div>fit &lt;- lm(Intention ~ AI_literacy + Trust + Support, data=df)</div>
                  <div>summary(fit)</div>
                </div>
                <div className="mt-3 flex gap-2">
                  <button className="flex-1 rounded-full bg-slate-900 py-2 text-xs font-bold text-white">Run Python</button>
                  <button className="flex-1 rounded-full border border-slate-200 py-2 text-xs font-bold">Copy R</button>
                </div>
              </div>
