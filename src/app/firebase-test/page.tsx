  ...
  return (
    <div className="min-h-screen bg-[#FFF8FB] p-8">
      <div className="max-w-2xl mx-auto bg-white border border-pink-100 rounded-2xl p-6 shadow-card">
        <h1 className="text-xl font-black">MONIRESH → Firebase</h1>
        <p className="text-sm text-ink-500 mt-1">projectId: <b>moniresh</b> • storage: moniresh.firebasestorage.app</p>
        <div className="mt-4 rounded-xl bg-moni-50 border border-moni-100 p-4 font-mono text-sm">{status}</div>
        {details && <pre className="mt-3 bg-ink-900 text-white p-4 rounded-xl text-xs overflow-auto">{JSON.stringify(details, null, 2)}</pre>}
        <div className="mt-4 text-xs text-ink-500">If you see “permission-denied”, go to Firebase Console → Firestore → Rules → set allow read/write for now, then we tighten with auth.</div>
      </div>
    </div>
  );
}
